...
class AllValuesDeferredObject(kromises:Array<Kromise<*, *>?>)
    : DeferredObject<AllValues, Throwable>()
        , Kromise<AllValues, Throwable> {
    ...
    fun <D, F>configureKromise(index: Int, kromise: Kromise<D, F>) {
        kromise.fail(object: FailCallback<F> {
            ...
            override fun onDone(result: D?) {
                synchronized(this@AllValuesDeferredObject) {
                    ...
                    values.set(index, OneResult(index, kromise, result!!))
